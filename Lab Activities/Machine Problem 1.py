...
# ---------- Rule 5: Computer Lab Usage ---------- #
def computer_lab_rule(student_name):
    print("\n--- Computer Lab Usage Rule Checker ---")
    valid_id = input("Does the student have a valid ID? (T/F): ").strip().upper() == "T"
    no_violations = input("Does the student have NO violations? (T/F): ").strip().upper() == "T"

    access_granted = valid_id and no_violations
    outcome = "Access Granted" if access_granted else "Access Denied"

    print(f"P = {tf(valid_id)} (Valid ID), Q = {tf(no_violations)} (No Violations)")
    print("Result:", outcome)

    log_result(student_name, "Computer Lab Usage Rule", outcome)
...
def main():
    ...
    while True:
        ...
        print("5) Computer Lab Usage Rule Checker")
        print("6) Exit")
        ...
        match choice:
            ...
            case "5": computer_lab_rule(student_name)
            case "6":
